...
func HandleRequests(server *gin.Engine, bookController BookController) {
	apiRoutes := server.Group("/library/api/v1/")
	{
		apiRoutes.GET("/books", func(ctx *gin.Context) {
			getAllBooks(ctx, bookController)
		})

		apiRoutes.GET("/books/:isbn", func(ctx *gin.Context) {
			getBookByIsbn(ctx, bookController)
		})

		apiRoutes.POST("/books", func(ctx *gin.Context) {
			addBook(ctx, bookController)
		})
	}
}

func getAllBooks(ctx *gin.Context, bookController BookController) {
	ctx.JSON(200, bookController.GetAll())
}

func getBookByIsbn(ctx *gin.Context, bookController BookController) {
	isbn := ctx.Param("isbn")
...
func addBook(ctx *gin.Context, bookController BookController) {
